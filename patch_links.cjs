...
// parse using a simple hack
let modifiedContent = content;

links.forEach(({ match, link }) => {
    // we can find the block that matches title and category
    // I will extract the file, split by '{', find match, and replace.
});

fs.writeFileSync('src/pages/WorksPage.tsx', modifiedContent);
